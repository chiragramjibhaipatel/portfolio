...
import {items as allTags} from '../data/custom_data';
import {useState, useCallback, useMemo, useEffect} from 'react';

export function ProjectTags ({tags, handleProjectChange}:{tags: string[]; handleProjectChange: (value: string[], id: string) => void}) {
    const [selectedTags, setSelectedTags] = useState<string[]>(tags);
    const [value, setValue] = useState('');
    const [suggestion, setSuggestion] = useState('');
    
    useEffect (() => {
        handleProjectChange(selectedTags, 'tags');
    }, [selectedTags]);
    ...
    const getAllTags = useCallback(() => {
        const savedTags = allTags.map((tag) => tag.name);
